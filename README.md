...
探索深度评分逻辑：
- **Local-only**：答案 grounded 于本地 + 充实 → 0.08（本地可答题的理想形态）；答案无本地支撑 → 0.04；本地无结果 → 0.02
- **有 web**：答案引用了 web 信息 → 0.10 + read_page 加成（最高 0.15）；搜了 web 但答案没用上（本地够却多搜）→ 0.05；web 无产出 → 0.03
- **设计意图**：按"答案是否引用 web 信息"判定 web 是否有效——本地够用时本地作答最划算（0.08 > 浪费 web 的 0.05）；本地不足时用上 web 最划算（0.10+ > 本地硬凑 0.08）
