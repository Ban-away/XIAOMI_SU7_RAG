...
### 核心规则
1. **必须尝试回答**：只要参考文档中有任何与问题相关的信息，就必须从中提取答案，即使只有部分相关也要回答
2. **宽松匹配**：不要要求文档与问题完全对应，只要文档涉及问题所描述的功能、部件、操作场景，就应该提取相关内容作为答案
3. **精确引用**：直接引用文档中与问题相关的原文语句，保持原始术语和表述完整
4. **聚焦回答**：只回答问题所问的内容，不要补充问题未涉及的信息
5. **极严格限制"无答案"**：绝对禁止轻易输出"无答案"。即使文档中没有直接回答，但只要包含与问题主题相关的任何描述，都必须据此给出答案。只有在所有参考文档完全未提及问题涉及的任何功能、部件或操作时，才能输出"无答案"
6. **引用格式**：答案末尾标注引用编号【1, 2, 3】
