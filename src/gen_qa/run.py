...
KEYWORDS_PROMPT_TPL = """你是一名专业的汽车领域NLP工程师，任务是从给定的文本片段中提取核心关键词。

严格规则：
1. 只能提取在下方文本中**实际出现**的词汇或短语，绝对不能凭联想生成文本中不存在的词
2. 如果一个词在文本中没有完整出现（例如文本中是“蓝牙手机”，你不能提取“蓝牙手机配对”），则不能提取
3. 优先提取文本中出现的汽车专业术语、产品型号、技术特征、关键动作
4. 关键词数量不超过5个
5. 如果文本中没有可提取的专业关键词，请直接输出“无”
...
if __name__ == "__main__":
    ...
    for unique_id, info in keywords_dict.items():
        ...
        kewyords = [item for item in keywords if item not in ["无", "SU7"]]
