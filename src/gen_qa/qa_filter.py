...
_IRRELEVANT_KEYWORDS = [
    # 诗词文学
    "诗", "诗歌", "古诗", "唐诗", "宋词", "诗经", "背一首",
    # 影视娱乐
    "还珠格格", "电视剧", "电影", "明星", "演员", "小品", "相声",
    "美剧", "神雕侠侣", "快乐大本营",
    # 音乐相关
    "放歌", "听歌", "播放", "放一首", "听他的", "听她的", "成名曲",
    "主题曲", "片头曲", "专辑",
    # 导航（非汽车场景的闲聊导航）
    "导航去", "怎么走", "导航过去",
    # 天气/金融/游戏
    "天气", "股票", "游戏", "汇率", "限行",
    # 明星名人
    "谢娜", "汪峰", "周杰伦", "王菲", "郭德纲", "郭麒麟", "肖战", "王一博",
    "张韶涵", "范玮琪", "费玉清", "昆凌", "何炅", "尚雯婕", "小虎队",
    # 地名景点
    "长城", "西湖", "故宫", "峨眉山", "三星堆", "外滩",
    # 通用知识
    "花语", "市花", "幸运色",
...
# 不完整短答案（以口语化词开头且很短）
_INCOMPLETE_ANSWER_RE = re.compile(r"^(还可以|没有[，,]|不行[，,]|也可以)[^。，；]{0,10}$")

# 纯娱乐类问题
_ENTERTAINMENT_Q_RE = re.compile(r"(歌曲$|歌手$|专辑$|相声$|小品$|京剧$)")
...
def is_low_quality(item: dict) -> Tuple[bool, str]:
    ...
    if _MULTI_QUESTION_RE.search(q):
        return True, "问题含多个问号(可能是拼接问题)"

    if _INCOMPLETE_ANSWER_RE.match(a):
        return True, "答案不完整(口语化短句)"

    if _ENTERTAINMENT_Q_RE.search(q):
        return True, "问题属娱乐类"
